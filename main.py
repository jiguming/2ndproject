...
from astropy.coordinates import SkyCoord, EarthLocation, AltAz, Moon # <--- Add Moon here
...
LOCATIONS_ASTRO = {
    "고요의 바다 (아폴로 11호 착륙지)": {
        # Change frame='moon' to frame=Moon()
...
with st.sidebar:
    ...
    # The frame='moon' in this context is for the target body of the transformation, not the coordinate system of SkyCoord directly.
    # It seems to be used to transform from a celestial frame to the Moon's body frame for the AltAz calculation.
    # This might work differently than the frame for SkyCoord on the Moon's surface.
    # Let's keep this as it is for now, as the error came from SkyCoord(lon=..., lat=..., frame='moon', ...)
    # If a new error arises here, we'll address it.
